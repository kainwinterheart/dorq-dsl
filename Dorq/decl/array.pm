...
sub exec
{
	...
	if( $var -> isa( 'Dorq::code::block' ) and not $var -> isa( 'Dorq::code::block::array_initializer' ) )
	{
		$var = Dorq::code::block::array_initializer -> new(
			\( $var -> val() )
		);
	}

	if( $var -> isa( 'Dorq::code::block::array_initializer' ) )
	{
		$var = $var -> exec( $context, 1 );

	}

	if( $var -> isa( 'Dorq::array' ) )
	{
